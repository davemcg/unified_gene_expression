...
test_summaries %>% 
  mutate(`Test #`=c(1,1,1,2,2,2,3,3,3,4,5,6,6,6)) %>% 
...
retina_rpe_GO_up %>% DT::datatable(options = list(pageLength = 20)) 
...
retina_rpe_GO_down %>%  DT::datatable(options = list(pageLength = 20)) 
```
...
cornea_vs_retina_rpe_GO_up %>%  DT::datatable(options = list(pageLength = 20)) 
# cornea vs RPE and retina (RPE and retina << cornea)
test1_contrast_genes <- contrast_DOWNgene_lists(big_six[[1]])
sets <- set_maker(test1_contrast_genes$Cornea.adult_vs_Retina.adult,
                  test1_contrast_genes$Cornea.adult_vs_RPE.adult,
...
cornea_vs_retina_rpe_GO_down %>%  DT::datatable(options = list(pageLength = 20)) 
...
coef2_up_GO %>% filter(`Expected Count` > 0.8) %>%  DT::datatable(options = list(pageLength = 20)) 
...
coef2_down_GO %>% filter(`Expected Count` > 0.8)
...
test4_up_go %>%  DT::datatable(options = list(pageLength = 20)) 
...
test4_down_go %>%  DT::datatable(options = list(pageLength = 20)) 
```
# 5. Adult Retina vs Stem Cell Line Retina
```{r, message =  FALSE}
colnames(big_six[[5]])
