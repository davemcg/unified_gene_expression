...
retina_rpe_GO_up %>% head(n=10) %>% FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("RPE > Retina"), colspan = c(8), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(retina_rpe_GO_up)) %>% 
...
retina_rpe_GO_down %>%  head(n=10) %>% FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("RPE < Retina"), colspan = c(8), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(retina_rpe_GO_down)) %>% 
  setFlexTableBorders(inner.vertical = no_border, inner.horizontal = no_border,
                      outer.vertical = no_border, outer.horizontal = big_border )
```
...
cornea_vs_retina_rpe_GO_up %>% head(n=10) %>% 
  FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("RPE and Retina > Cornea"), colspan = c(8), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(cornea_vs_retina_rpe_GO_up)) %>% 
  setFlexTableBorders(inner.vertical = no_border, inner.horizontal = no_border,
                      outer.vertical = no_border, outer.horizontal = big_border )
...
cornea_vs_retina_rpe_GO_down %>% head(n=10) %>% 
  FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("RPE and Retina < Cornea",""), colspan = c( 4, 4 ), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(cornea_vs_retina_rpe_GO_down)) %>% 
  setFlexTableBorders(inner.vertical = no_border, inner.horizontal = no_border,
                      outer.vertical = no_border, outer.horizontal = big_border ) 
...
coef2_up_GO %>% filter(`Expected Count` > 0.8) %>%  head(n=10) %>%  FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("Adult RPE > Stem Cell RPE"), colspan = c(8), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(coef2_up_GO)) %>% 
  setFlexTableBorders(inner.vertical = no_border, inner.horizontal = no_border,
                      outer.vertical = no_border, outer.horizontal = big_border )
...
coef2_down_GO %>% filter(`Expected Count` > 0.8) %>%  head(n=10) %>%  FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("Adult RPE < Stem Cell RPE"), colspan = c(8), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(coef2_down_GO)) %>% 
  setFlexTableBorders(inner.vertical = no_border, inner.horizontal = no_border,
                      outer.vertical = no_border, outer.horizontal = big_border )
...
test4_up_go %>%  head(n=10) %>%  FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("Stem Cell RPE < Immortalized Cell Line RPE"), colspan = c(8), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(test4_up_go)) %>% 
  setFlexTableBorders(inner.vertical = no_border, inner.horizontal = no_border,
                      outer.vertical = no_border, outer.horizontal = big_border )
...
test4_down_go %>%  head(n=10) %>%  FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("Stem Cell RPE < Immortalized Cell Line RPE"), colspan = c(8), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(test4_up_go)) %>% 
...
test5_up_go %>%   head(n=10) %>%  FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("Stem Cell Retina > Adult Retina"), colspan = c(8), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(test5_up_go)) %>% 
  setFlexTableBorders(inner.vertical = no_border, inner.horizontal = no_border,
                      outer.vertical = no_border, outer.horizontal = big_border )

test5_down_go <- GO_enrichment(test5_contrast_genes_down$Retina.adult_vs_Retina.stemcell, background_genes ,'BP')
test5_down_go %>%  head(n=10) %>%  FlexTable(body.cell.props = cellProperties( padding = 2 ), 
                        header.par.props = parProperties(text.align = "left" ), 
                        body.par.props = parProperties(text.align = 'left'), header.columns = FALSE) %>%
  addHeaderRow(value = c("Stem Cell Retina < Adult Retina"), colspan = c(8), par.properties = parProperties(text.align='left')) %>% 
  addHeaderRow(value = colnames(test5_down_go)) %>% 
  setFlexTableBorders(inner.vertical = no_border, inner.horizontal = no_border,
                      outer.vertical = no_border, outer.horizontal = big_border )
```
# 6. Adult Cornea vs Fetal Cornea vs Cell Line Cornea
```{r, message= FALSE}
topTable(big_six[[6]],coef=1, p.value = 0.05) %>%  DT::datatable(options = list(pageLength = 20)) 
```
