...
test4_down_go <- test4_down_go %>% mutate(Test='Test D', Set='a',Direction='Down',Contrasts=colnames(big_six[[4]]))
...
test5_down_go <- test5_down_go %>% mutate(Test='Test E', Set='a',Direction='Down',Contrasts=colnames(big_six[[5]]))
...
```{r, message = FALSE}
go_enrichment %>% group_by(Test, Set, Direction)  %>% summarise(Count=sum(as.numeric(`P value (FDR)`) < 0.05))
```
