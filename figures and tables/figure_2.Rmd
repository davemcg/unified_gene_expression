...
cluster_centers <- tsne_plot %>% 
  left_join(.,core_tight) %>% group_by(Clusters) %>%
...
center_samples <- tsne_plot %>% left_join(.,core_tight)  %>%
    left_join(.,cluster_centers, by=c('Clusters')) %>% 
...
cluster_stats <- tsne_plot %>% left_join(.,core_tight)  %>% 
  ...
  group_by(Cluster) %>% 
  summarise(Cluster_Tissues = paste(unique(Tissue), collapse=', '), Cluster_Counts = n())
...
ggplot(tsne_plot_prep,aes(x=X1,y=X2)) + 
  ...
  geom_point(size=10, alpha=0.2, aes(x=X1, y=X2, colour=Cluster)) +
  geom_point(size=4, alpha=0.6, aes(shape=Origin)) + 
  geom_text_repel(data=subset(tsne_plot_prep, sample_accession %in% center_samples), aes(label=Label)) +
