...
Heatmap of t-sne distances for pan-tissue. Aggregated across 5 to 50 perplexities and at the tissue level
...
mean_coord <- long_tsne_plot %>% 
  ...
  group_by(Tissue) %>%summarise(X1=mean(X1),X2=mean(X2)) %>%  # then group by tissue and summarise across all samples in a tissue
  data.frame()
