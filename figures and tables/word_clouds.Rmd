...
```{r, message = FALSE}
all_keywords <-go_enrichment %>% dplyr::select(`GO BP ID`, Term) %>% unique() %>% .[['Term']]
...
cloud_maker <- function(word_vector, exclusion_words, max.words, all_go){
  ...
  set.seed(1234)
  wordcloud(words = calculated$word, scale=c(3,1),freq = calculated$transformed_delta, min.freq = 1,
...
All contrasts
```{r, message = FALSE}
go_enrichment %>% dplyr::select(Contrasts, Direction) %>% unique()
```
## RPE vs Retina
### Up
