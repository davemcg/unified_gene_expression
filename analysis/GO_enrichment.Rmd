...
```{r, message = FALSE}
all_go_tester <- function(test_set, test_name){
...
testA_go_all_sets <- all_go_tester(set_maker2(test1_comps),'Test A')
...
# 2. ESC vs RPE vs Retina (all Stem cell lines, non immortalized)
```{r, message = FALSE}
testB_go_all_sets <- all_go_tester(set_maker2(test2_comps),'Test B')
```

# 3. Adult RPE vs Fetal RPE vs Cell Line RPE
```{r}
testC_go_all_sets <- all_go_tester(set_maker2(test3_comps),'Test C')
...
# 6. Adult Cornea vs Fetal Cornea vs Cell Line Cornea
```{r, message = FALSE}
testF_go_all_sets <- all_go_tester(set_maker2(test6_comps),'Test F')
```
...
Generate GO enrichment for all tests
```{r, message = FALSE}
go_up <- contrast_UPgene_lists(efit_all)
go_down <- contrast_DOWNgene_lists(efit_all)
