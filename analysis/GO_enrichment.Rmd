...
all_go_tester <- function(limma_data, test_name){
  column_classes <- rep(x='character',13)
  column_names <- c("GO ID","P value","P value (FDR)","Odds Ratio","Expected Count","Count","Size","Term","Test","Set","Direction", "Contrasts", "Ontology")
  ...
  for (i in names(up_expressed_sets)){
    if (length(up_expressed_sets[[i]])<6){
      ...
      for (ontology in c('BP','MF')){
        go_run <- GO_enrichment(up_expressed_sets[[i]], background_genes, ontology)
        go_run <- go_run %>% data.frame() %>% mutate(Test=test_name, Set=i,Direction='Up',Contrasts=paste(contrasts, collapse=','), Ontology=ontology)
  ...
  for (i in names(down_expressed_sets)){
    if (length(down_expressed_sets[[i]])<6){
      ...
      print(i)
      for (ontology in c('BP','MF')){
        go_run <- GO_enrichment(down_expressed_sets[[i]], background_genes, ontology)
        go_run <- go_run %>% data.frame() %>% mutate(Test=test_name, Set=i,Direction='Down',Contrasts=paste(contrasts, collapse=','), Ontology=ontology)
        colnames(go_run)[1] <- 'GO.ID'
        all_go_terms <- rbind(all_go_terms, go_run)
      }
    }
  }
colnames(all_go_terms) <- c("GO ID","P value","P value (FDR)","Odds Ratio","Expected Count","Count","Size","Term","Test","Set","Direction", "Contrasts", "Ontology")
all_go_terms
}
```
...
test4_up_go <- rbind(GO_enrichment(test4_contrast_genes_up, background_genes, 'BP') %>% 
                       mutate(Test='Test D', Set='a',Direction='Up',Contrasts=colnames(big_six[[4]]), Ontology='BP') %>% 
                         rename(`GO BP ID`='GO ID'),
                     GO_enrichment(test4_contrast_genes_up, background_genes ,'MF') %>% 
                       mutate(Test='Test D', Set='a',Direction='Up',Contrasts=colnames(big_six[[4]]), Ontology='MF') %>% 
...
test4_down_go <- rbind(GO_enrichment(test4_contrast_genes_down, background_genes, 'BP') %>% 
                       mutate(Test='Test D', Set='a',Direction='Down',Contrasts=colnames(big_six[[4]]), Ontology='BP') %>% 
                         rename(`GO BP ID`='GO ID'),
                     GO_enrichment(test4_contrast_genes_down, background_genes ,'MF') %>% 
                       mutate(Test='Test D', Set='a',Direction='Down',Contrasts=colnames(big_six[[4]]), Ontology='MF') %>% 
                         rename(`GO MF ID`='GO ID'))
...
test5_up_go <- rbind(GO_enrichment(test5_contrast_genes_up, background_genes, 'BP') %>% 
                       mutate(Test='Test E', Set='a',Direction='Up',Contrasts=colnames(big_six[[5]]), Ontology='BP') %>% 
                         rename(`GO BP ID`='GO ID'),
                     GO_enrichment(test5_contrast_genes_up, background_genes ,'MF') %>% 
                       mutate(Test='Test E', Set='a',Direction='Up',Contrasts=colnames(big_six[[5]]), Ontology='MF') %>% 
                         rename(`GO MF ID`='GO ID'))
#go_printer(test5_up_go,8, 'Stem Cell Retina > Adult Retina')

test5_down_go <- rbind(GO_enrichment(test5_contrast_genes_down, background_genes, 'BP') %>% 
                       mutate(Test='Test E', Set='a',Direction='Down',Contrasts=colnames(big_six[[5]]), Ontology='BP') %>% 
                         rename(`GO BP ID`='GO ID'),
                     GO_enrichment(test5_contrast_genes_down, background_genes ,'MF') %>% 
                       mutate(Test='Test E', Set='a',Direction='Down',Contrasts=colnames(big_six[[5]]), Ontology='MF') %>% 
                         rename(`GO MF ID`='GO ID'))
...
```{r, message = FALSE}
go_enrichment %>% group_by(Test, Set, Ontology,Direction)  %>% summarise(Count=sum(as.numeric(`P value (FDR)`) < 0.05))
```
