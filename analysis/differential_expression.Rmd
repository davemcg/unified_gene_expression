...
QC steps, removing low count genes and doing qsmooth
```{r, message = FALSE}
# density plot, pre low gene count removal and qsmooth(?)
gather_lST<-gather(txi.eye.lsTPM,sample_accession) %>% left_join(.,core_tight)
...
table(rowSums(txi.eye.lsTPM)<(ncol(txi.eye.lsTPM)))
txi.eye.lsTPM_geneRemoval <- txi.eye.lsTPM[(rowSums(txi.eye.lsTPM)>ncol(txi.eye.lsTPM)),]

# re-do density plot
gather_lST<-gather(txi.eye.lsTPM_geneRemoval,sample_accession) %>% left_join(.,core_tight)
ggplot(gather_lST,aes(x=log2(value+1),group=sample_accession))+geom_density()+facet_wrap(~Tissue)+coord_cartesian(ylim=c(0,0.5)) + theme_Publication()

# qsmooth
qs <- qsmooth(object = txi.eye.lsTPM_geneRemoval,groupFactor = as.factor(SampleTable_eye$Tissue))
lsTPM_qsmooth <- qsmoothData(qs)

# re-redo density plot
gather_lST<-gather(data.table(lsTPM_qsmooth),sample_accession) %>% left_join(.,core_tight)
ggplot(gather_lST,aes(x=log2(value+1),group=sample_accession))+geom_density()+facet_wrap(~Tissue)+coord_cartesian(ylim=c(0,0.5)) + theme_Publication()
```
...
cont.matrix <- makeContrasts(RPE.adult_vs_Retina.adult="RPE_Adult.Tissue-Retina_Adult.Tissue",
                             RPE.adult_vs_RPE.cell="RPE_Adult.Tissue-RPE_Cell.Line",
