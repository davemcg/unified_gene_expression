...
colnames(txi.eye.lsTPM) <- eye_samples
```
...
2. ESC vs RPE vs Retina (all cell lines, non hTERT)
3. Adult RPE vs Fetal RPE vs Cell Line RPE
4. hTERT RPE vs Cell Line RPE
5. Adult Retina vs Cell Line Retina
6. Adult Cornea vs Fetal Cornea
...
cont.matrix <- makeContrasts(Cornea.adult_vs_Retina.adult='Cornea_Adult.Tissue-Retina_Adult.Tissue',
                             ...
                             levels=design)
# fitting time
vfit.cont <- contrasts.fit(vfit, contrasts=cont.matrix)
efit <- eBayes(vfit.cont)
# DE
DE <- topTableF(efit,number=30000, adjust.method = 'fdr', lfc = 1) %>% filter(adj.P.Val<0.05) %>% data.frame()
```
...
cont.matrix <- makeContrasts(RPE.adult_vs_RPE.cell="RPE_Adult.Tissue-RPE_Cell.Line",
                            ...
                            #RPE_vs_Retina="(RPE_Adult.Tissue+RPE_Cell.Line)/2-(Retina_Adult.Tissue+Retina_Cell.Line)/2",
                             levels=design)
...
summary(decideTests(efit, lfc = 1, p.value = 0.05, adjust.method = 'fdr'))
```
...
```{r, message = FALSE}
vfit_all <- lmFit(v, design)
vfit_all <- contrasts.fit(vfit_all, contrasts=cont.matrix_all)
efit_all <- eBayes(vfit_all)
DE <- topTableF(efit_all,number=300000, adjust.method = 'fdr', lfc = 2) %>% filter(adj.P.Val<0.01) %>% data.frame()
```
