...
```{r, message = FALSE}
 set.seed(3939)
 wss <- (nrow(DE_55[,1:55])-1)*sum(apply(DE_55[,1:55],2,var))
 for (i in 2:80) wss[i] <- sum(kmeans(DE_55[,1:55],
...
```{r, message = FALSE}
 ...
 kfit <- kmeans(DE[,2:37], 20, iter.max = 10000, algorithm="MacQueen") # 20 cluster solution
# # move k means cluster assignment over to DE data frame
 DE$cluster <- kfit$cluster
 # clean up the contrast names
 comparisons <- colnames(DE)[2:55] %>% 
   gsub('_vs_','|vs|', .) %>% 
   gsub('_',' (',.) %>% 
   gsub('\\.Line|\\.Tissue',')',.) %>% 
   gsub('\\|vs\\|',' vs ',.) %>% 
   gsub('hTERT\\.Cell','hTERT',.)
 colnames(DE)[2:55] <- comparisons
```
...
superheat(DE %>% dplyr::select(2:56),
          ...
          bottom.label.size = 0.3, left.label.size=0.02, 
