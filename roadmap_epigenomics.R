...
# grab SRAdb metadata to merge into core_data sqlite3
sra_fields <- c('experiment_accession','sample_alias', 'submission_accession', 'run_accession','library_layout','instrument_model')
run_info <- read.table(text='',col.names = sra_fields)
...
roadmap_core <- left_join(run_info,grab)
...
ebi_fastq_link_builder <- function(srr) {
...
roadmap_core$fastq_ftp <- sapply(roadmap_core$run_accession, function(x) ebi_fastq_link_builder(x))
