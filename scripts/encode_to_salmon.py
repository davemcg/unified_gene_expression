...
encode_sample_ID = encode_sample_ID
...
try:
	for i in re.split('\|\||,',encode_sample_ID):
...
if library == 'paired':
  forward_reads = [x.split(',')[0] for x in encode_fastq_url.split('||')]
  reverse_reads = [x.split(',')[1] for x in encode_fastq_url.split('||')]
  
  forward_reads = ' '.join([temp_folder + x for x in forward_reads])
  reverse_reads = ' '.join([temp_folder + x for x in reverse_reads])
	salmon_call = 'sbatch --cpus-per-task 8 ~/git/unified_gene_expression/scripts/run_salmon.sh ' + \
				  '\"' + forward_reads + '\"\ ' + '\"' + reverse_reads + '\"\' + \
           'paired ' + salmon_main_dir + '/' + encode_sample_ID
	subprocess.check_call(salmon_call, shell=True)
if library == 'single':
    reads = [x.split(',')[0] for x in encode_fastq_url.split('||')]
